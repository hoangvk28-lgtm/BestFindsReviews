...
- **Site name:** BestFindsReviews (`SITE_NAME = "BestFindsReviews"`)
- **Live domain:** `https://www.bestfindsreviews.com` (www is canonical; non-www redirects 301)
...
- **Content types:**
  ...
  - Static editorial pages — `/how-we-review`, `/about-bestfindsreviews`, `/affiliate-disclosure`, `/privacy-policy`, `/contact`
...
- **Twitter handle:** `@bestfindsreviews`
...
├── public/
│   ├── logo-bestfindsreviews.png / .svg / logo-bestfindsreviews-official.png
│   ├── icon.webp
│   ├── hero-banner.jpg
...
- **Canonical domain is always:** `https://www.bestfindsreviews.com` (www, no bare `bestfindsreviews.com`)
- **Non-www redirect** is handled in `next.config.ts`:
  ```ts
  // Matches host: bestfindsreviews.com → redirects to https://www.bestfindsreviews.com/:path*
  // permanent: true → HTTP 301
  ```
- **NEXT_PUBLIC_SITE_URL** must be set to `https://www.bestfindsreviews.com` in production — `lib/seo.ts` strips trailing slashes: `process.env.NEXT_PUBLIC_SITE_URL?.replace(/\/$/, "")`
...
- **All schema, sitemap, and OG URLs** must use the `www.bestfindsreviews.com` domain
- **Legacy redirect:** `/about` → `/about-bestfindsreviews` (301, in `next.config.ts`)
...
```ts
buildMetadata({
  title: string,        // Do NOT append "| BestFindsReviews" — the function does it
...
2. **`howWeEvaluated[]`** (required as of 2026-07-18) → renders as **"How We Evaluated These [Products]"**, a 2-column card grid, positioned **right after** the product reviews (before "How to Choose"). 4-5 entries describing the guide-specific scoring methodology — category-specific angles (e.g. Stability, Adjustability, Build quality, Device compatibility, Value for price for a tablet-stand guide), not the generic site-wide "BestFindsReviews Fit Score" weights from the Scoring Criteria table above reused verbatim.
   ```ts
...
- Use "BestFindsReviews" as the brand name — never "Desk Finds" (two words) or "bestfindsreviews" (all lowercase)
...
- 4-6 sentences total per product description
...
### Score Label
- The editorial score is called the **"BestFindsReviews Fit Score"** — use this exact label in UI and content
...
### Schema Restrictions
- **Do not** emit `AggregateRating` schema for BestFindsReviews Fit Scores — they are editorial, not crowd-sourced ratings
...
- Logo: `alt="BestFindsReviews logo"`
...
- Sitemap URL: `https://www.bestfindsreviews.com/sitemap.xml`
...
- **`/about` → `/about-bestfindsreviews` redirect** added to `next.config.ts` with `permanent: true`
- **Double title suffix bug fixed** — `buildMetadata()` uses `{ absolute: fullTitle }` to prevent `"Title | BestFindsReviews | BestFindsReviews"`
...
All commands run from the `bestfindsreviews/` directory.
...
4. Manually verify the changed page's `<title>` tag does not contain `| BestFindsReviews | BestFindsReviews`
...
|---|---|---|
| `ADMIN_EMAIL` | Admin login email | `admin@bestfindsreviews.com` |
...
| `NEXT_PUBLIC_SITE_URL` | Canonical site URL | `https://www.bestfindsreviews.com` |

### Secrets Policy — Critical
- **Never commit `.env.local`** — it contains live Supabase service role keys and admin credentials
- `.env.example` in `bestfindsreviews/` is the template — keep it updated but with placeholder values only
...
- Do not change the canonical domain from `www.bestfindsreviews.com` to anything else
...
### Metadata
- [ ] Page `<title>` does not contain `| BestFindsReviews | BestFindsReviews` (double suffix)
