...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildMetadata({
    title: `${author.name} — ${author.role} | BestFindsReviews`,
...
export default async function AuthorPage({ params }: Props) {
  ...
  const authorGuides = allGuides.filter(
    (g) =>
      ...
      g.author?.toLowerCase().includes("bestfindsreviews")
  );
  ...
  const jsonLd = author.isPerson
    ? {
        ...
        worksFor: { "@type": "Organization", name: "BestFindsReviews", url: SITE_URL },
