...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-bagless-self-emptying-robot-vacuums";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-bagless-self-emptying-robot-vacuums",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-bagless-self-emptying-robot-vacuums");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-bagless-self-emptying-robot-vacuums` },
    about: [{"@type":"Thing","name":"Best Bagless Self-Emptying Robot Vacuums"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Bagless Self-Emptying Robot Vacuums", item: `${SITE_URL}/guide/best-bagless-self-emptying-robot-vacuums` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-bagless-self-emptying-robot-vacuums#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Bagless Self-Emptying Robot Vacuums</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Bagless Self-Emptying Robot Vacuums" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A self-emptying robot vacuum dock normally means a sealed dust bag that you replace every few months without touching the dirt inside it. Bagless self-empty docks skip that disposable bag entirely, collecting debris in a reusable bin that you empty by hand into the trash. That trade removes an ongoing bag cost from the equation, but it shifts the work and the dust exposure back onto the owner.</p>
          <p>We researched eight current bagless self-empty models and focused on the details that matter once the novelty wears off: how much fine dust escapes when you pull the bin, how often the cyclone path and dock seals need attention, and whether the filter cleaning routine actually keeps suction consistent over months of use rather than just on day one.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Bagless Self-Emptying Robot Vacuum</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Bagless Self-Emptying Robot Vacuums</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Bagless Self-Emptying Robot Vacuums</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=bagless+self-emptying+robot+vacuums&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
