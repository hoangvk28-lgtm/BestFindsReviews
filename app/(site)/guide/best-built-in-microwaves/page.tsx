...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-built-in-microwaves";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-built-in-microwaves",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-built-in-microwaves");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-built-in-microwaves` },
    about: [{"@type":"Thing","name":"Best Built-In Microwaves"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Built-In Microwaves", item: `${SITE_URL}/guide/best-built-in-microwaves` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-built-in-microwaves#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Built-In Microwaves</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Built-In Microwaves" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Best Built-In Microwaves can look similar on a spec sheet but differ in ways that matter once one is actually installed and used day to day: cutout dimensions, ventilation, and trim kit are the details most listings gloss over in favor of headline wattage and capacity numbers. In this lineup alone, rated output ranges from 1000W on the KoolMore 24 Inch Stainless-Steel Microwave Drawer with 1.2 cu. ft. up to 1200W on the Panasonic Microwave Oven NN-SN686S Stainless Steel, a real difference in heating speed that a single star rating won't tell you.</p>
          <p>We compared all 8 models below, spanning Sharp SMC2266KS 2.2 cu. ft. 1200W Stainless Steel Countertop at $229.99 up to Sharp SMD2470ASY 24 in. 1.2 cu. ft. Built-In Microwave Drawer Oven at $1497.00, with a focus on separating true built-in microwaves from countertop models explicitly approved for trim-kit installation, and cross-checked electrical access and replacement fit against the current listing data rather than repeating manufacturer marketing claims.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Built-In Microwave</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Built-In Microwaves</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Built-In Microwaves</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=built-in+microwaves&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
