...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-lawn-mowers-under-150";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-lawn-mowers-under-150",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-lawn-mowers-under-150");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-lawn-mowers-under-150` },
    about: [{"@type":"Thing","name":"Best Lawn Mowers Under $150"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Lawn Mowers Under $150", item: `${SITE_URL}/guide/best-lawn-mowers-under-150` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-lawn-mowers-under-150#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Lawn Mowers Under $150</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Lawn Mowers Under $150" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Under $150 is a thin tier for powered mowers. Most full-size gas and battery mowers with a real deck and self-propel start well above this price, so this list leans on a manual reel mower, compact corded electrics, and a couple of small cordless push mowers with limited review history. We priced every model at or below $150 as sold, not on a temporary sale.</p>
          <p>If you have a larger or uneven lawn, or you want more cutting width and a bigger battery, our Best Lawn Mowers Under $100 and Best Lawn Mowers Under $200 guides cover the tiers directly below and above this one, and the Under $200 guide in particular opens up more self-propelled and higher-capacity cordless options.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Lawn Mowers Under $150</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Lawn Mowers Under $150</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Lawn Mowers Under $150</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=lawn+mowers+under+150&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
