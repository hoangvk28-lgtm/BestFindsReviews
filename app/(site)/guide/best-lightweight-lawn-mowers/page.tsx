...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-lightweight-lawn-mowers";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-lightweight-lawn-mowers",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-lightweight-lawn-mowers");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-lightweight-lawn-mowers` },
    about: [{"@type":"Thing","name":"Best Lightweight Lawn Mowers"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Lightweight Lawn Mowers", item: `${SITE_URL}/guide/best-lightweight-lawn-mowers` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-lightweight-lawn-mowers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Lightweight Lawn Mowers</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Lightweight Lawn Mowers" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A published weight on the box rarely tells the whole story. Reel mowers list a bare frame weight, corded electrics list the mower without a cord, and cordless mowers often list the deck alone before the battery goes in. The number that actually matters when you lift, tilt, or maneuver the mower is the ready-to-mow weight, meaning battery, bag, and any attached accessory included.</p>
          <p>We compared how each mower real category (manual, corded, cordless, or combo tool) affects that ready-to-mow weight, how the weight is distributed for lifting and storage, and whether a light frame trades away push stability or bagging capacity. That is a more useful way to shortlist than sorting by the number printed in the title.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Lightweight Lawn Mower</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Lightweight Lawn Mowers</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Lightweight Lawn Mowers</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=lightweight+lawn+mowers&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
