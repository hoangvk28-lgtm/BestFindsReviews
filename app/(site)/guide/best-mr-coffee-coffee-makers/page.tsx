...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-mr-coffee-coffee-makers";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-mr-coffee-coffee-makers",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-mr-coffee-coffee-makers");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-mr-coffee-coffee-makers` },
    about: [{"@type":"Thing","name":"Best Mr Coffee Coffee Makers"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Mr Coffee Coffee Makers", item: `${SITE_URL}/guide/best-mr-coffee-coffee-makers` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-mr-coffee-coffee-makers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Mr Coffee Coffee Makers</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Mr Coffee Coffee Makers" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Mr Coffee roundups usually lead with low price and basic programmability, and often mix current models with discontinued ones that are no longer sold new. That makes it hard to know which listing is actually the current version of a given machine.</p>
          <p>We compared four current Mr Coffee model numbers on real small-batch brewing quality versus a full pot, the drip risk of pulling a cup mid-brew, how long the warming plate can scorch coffee if left sitting, and what a replacement carafe actually costs when the glass eventually breaks or the machine outlives its stock lid.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Mr Coffee Coffee Maker</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Mr Coffee Coffee Makers</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Mr Coffee Coffee Makers</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=coffee+makers+mr+coffee&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
