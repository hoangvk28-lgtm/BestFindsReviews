...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-variable-temperature-electric-kettles";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-variable-temperature-electric-kettles",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-variable-temperature-electric-kettles");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-variable-temperature-electric-kettles` },
    about: [{"@type":"Thing","name":"Best Variable-Temperature Electric Kettles"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Variable-Temperature Electric Kettles", item: `${SITE_URL}/guide/best-variable-temperature-electric-kettles` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-variable-temperature-electric-kettles#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Variable-Temperature Electric Kettles</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Variable-Temperature Electric Kettles" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A long preset list does not automatically make a temperature control electric kettle more useful. The details that matter are the temperatures you can actually select, what happens after the kettle reaches its target, whether lifting the kettle erases your settings, and how comfortably you can fill and pour it every day.</p>
          <p>We researched five current models surfaced and verified through Amazon Creators API, then checked their control claims against available manufacturer documentation. WorthRated has not independently measured temperature accuracy, overshoot, recovery, hold-mode energy use, or long-term durability, so those points are labeled as manufacturer claims or open verification items rather than presented as hands-on test results.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Variable-Temperature Electric Kettle</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Variable-Temperature Electric Kettles</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Variable-Temperature Electric Kettles</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=temperature+control+electric+kettle&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
