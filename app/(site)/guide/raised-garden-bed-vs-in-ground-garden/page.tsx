...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/raised-garden-bed-vs-in-ground-garden";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/raised-garden-bed-vs-in-ground-garden",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("raised-garden-bed-vs-in-ground-garden");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/raised-garden-bed-vs-in-ground-garden` },
    about: [{"@type":"Thing","name":"Raised Garden Bed vs In-Ground Garden"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Raised Garden Bed vs In-Ground Garden", item: `${SITE_URL}/guide/raised-garden-bed-vs-in-ground-garden` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/raised-garden-bed-vs-in-ground-garden#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Raised Garden Bed vs In-Ground Garden</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Raised Garden Bed vs In-Ground Garden" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Raised Garden Bed vs In-Ground Garden can look similar in a listing photo but differ in ways that matter once soil, water, and a full season of weather are involved: Installed cost, Soil improvement, and Water use are the details most listings gloss over in favor of a generic product shot. Height alone in this lineup spans from 12 inches on the FOYUEE 2-Pack Galvanized Raised Garden Bed 8x4x1ft up to 24 inches on the Best Choice Products 6x3x2ft Outdoor Metal Raised Garden Bed, a real difference in how much bending a gardener has to do.</p>
          <p>We compared all 8 models below, spanning Foxlang 8x4x1ft(2 PCS) Galvanized Raised Garden Bed at $30.99 up to Land Guard 3 Pack 8×4×1ft Galvanized Metal Raised Garden Bed at $99.99, with a focus on calculating installed cost for bed, soil, edging, irrigation and paths versus improving native soil, and cross-checked Weed control and Maintenance against the current listing data rather than repeating manufacturer marketing claims.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Raised Garden Bed vs In-Ground Garden</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Raised Garden Bed vs In-Ground Garden</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Raised Garden Bed vs In-Ground Garden</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=Raised+Garden+Bed+vs+In-Ground+Garden&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
