...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-cuisinart-coffee-makers";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-cuisinart-coffee-makers",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-cuisinart-coffee-makers");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-cuisinart-coffee-makers` },
    about: [{"@type":"Thing","name":"Best Cuisinart Coffee Makers"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Cuisinart Coffee Makers", item: `${SITE_URL}/guide/best-cuisinart-coffee-makers` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-cuisinart-coffee-makers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Cuisinart Coffee Makers</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Cuisinart Coffee Makers" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Cuisinart roundups tend to repeat the same headline numbers, 12 or 14 cup capacity, programmability, brew strength control, and self-clean, without checking what a manufacturer cup actually measures or how the model numbers differ from one another.</p>
          <p>We compared four current, distinct Cuisinart models covering a classic 12-cup programmable, a PerfecTemp glass carafe model, a PerfecTemp thermal carafe model, and a single-serve pod machine, and looked specifically at actual brewed ounces versus the cup-count marketing, small-batch performance, lid and pouring usability, and whether the self-clean alert genuinely reduces scale buildup or mostly adds a feature line to the box.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Cuisinart Coffee Maker</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Cuisinart Coffee Makers</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Cuisinart Coffee Makers</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=reviews+on+cuisinart+coffee+makers&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
