...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-brushless-lawn-mowers";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-brushless-lawn-mowers",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-brushless-lawn-mowers");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-brushless-lawn-mowers` },
    about: [{"@type":"Thing","name":"Best Brushless Lawn Mowers"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Brushless Lawn Mowers", item: `${SITE_URL}/guide/best-brushless-lawn-mowers` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-brushless-lawn-mowers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Brushless Lawn Mowers</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Brushless Lawn Mowers" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A brushless motor claim tells you the motor design, not how the whole mower performs. Some listings put the word in the title to stand out, so we checked each product title directly for a brushless claim before including it here, and left out mowers that only imply the feature without stating it.</p>
          <p>Brushless alone is not automatic proof of overall mower quality. Deck width, battery capacity, controller protection, and how the machine holds up under tall or damp grass matter just as much as the motor label, so we compare the complete mower rather than ranking by motor type alone.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Brushless Lawn Mower</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Brushless Lawn Mowers</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Brushless Lawn Mowers</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=brushless+lawn+mowers&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
