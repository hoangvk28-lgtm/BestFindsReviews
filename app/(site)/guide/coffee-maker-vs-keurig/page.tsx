...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/coffee-maker-vs-keurig";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/coffee-maker-vs-keurig",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("coffee-maker-vs-keurig");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/coffee-maker-vs-keurig` },
    about: [{"@type":"Thing","name":"Coffee Maker vs Keurig"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Coffee Maker vs Keurig", item: `${SITE_URL}/guide/coffee-maker-vs-keurig` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/coffee-maker-vs-keurig#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Coffee Maker vs Keurig</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Coffee Maker vs Keurig" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A drip coffee maker and a Keurig-style pod machine solve the same basic problem in opposite ways. Drip machines brew a full batch from bulk ground coffee in one cycle; pod machines brew one cup at a time from sealed, pre-measured pods with almost no setup. The right choice depends less on which brews better coffee and more on the actual cup-size and consistency needs of your household.</p>
          <p>We compared two current drip machines and two current Keurig pod machines on the details that matter for this specific decision: per-cup cost, how brew temperature and speed hold up across repeat cups, descaling and maintenance access, and the tradeoff of pod convenience against pod waste, stated plainly as a factor some buyers weigh rather than a moral judgment either way.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Coffee Maker vs Keurig</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Coffee Maker vs Keurig</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Coffee Maker vs Keurig</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=coffee+maker+vs+keurig&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
