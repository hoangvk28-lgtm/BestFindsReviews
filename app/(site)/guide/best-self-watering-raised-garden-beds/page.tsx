...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-self-watering-raised-garden-beds";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-self-watering-raised-garden-beds",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-self-watering-raised-garden-beds");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-self-watering-raised-garden-beds` },
    about: [{"@type":"Thing","name":"Best Self-Watering Raised Garden Beds"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Self-Watering Raised Garden Beds", item: `${SITE_URL}/guide/best-self-watering-raised-garden-beds` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-self-watering-raised-garden-beds#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Self-Watering Raised Garden Beds</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Self-Watering Raised Garden Beds" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Best Self-Watering Raised Garden Beds can look similar in a listing photo but differ in ways that matter once soil, water, and a full season of weather are involved: Reservoir volume, Indicator accuracy, and Overflow are the details most listings gloss over in favor of a generic product shot. Height alone in this lineup spans from 16 inches on the Vego garden 8' x 8' L-Shaped Woodgrain Finish Metal Raised Garden Bed up to 17 inches on the Vego garden Large 9-in-1 Modular Metal Raised Garden Bed Kit, a real difference in how much bending a gardener has to do.</p>
          <p>We compared all 5 models below with a focus on measuring usable reservoir volume and days between refills under stated weather, and cross-checked Aeration and Cleaning against the current listing data rather than repeating manufacturer marketing claims.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Self-Watering Raised Garden Bed</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Self-Watering Raised Garden Beds</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Self-Watering Raised Garden Beds</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=Self-Watering+Raised+Garden+Beds&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
