...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-microwaves-with-large-buttons";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-microwaves-with-large-buttons",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-microwaves-with-large-buttons");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-microwaves-with-large-buttons` },
    about: [{"@type":"Thing","name":"Best Microwaves With Large Buttons"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Microwaves With Large Buttons", item: `${SITE_URL}/guide/best-microwaves-with-large-buttons` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-microwaves-with-large-buttons#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Microwaves With Large Buttons</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Microwaves With Large Buttons" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Best Microwaves With Large Buttons can look similar on a spec sheet but differ in ways that matter once one is actually installed and used day to day: button dimensions, contrast, and press force are the details most listings gloss over in favor of headline wattage and capacity numbers. In this lineup alone, rated output ranges from 700W on the Commercial Chef 0.6 Cu. Ft. Countertop Microwave Oven up to 800W on the Nostalgia Retro Countertop Microwave Oven - Large 800-Watt - 0.9 cu, a real difference in heating speed that a single star rating won't tell you.</p>
          <p>We compared all 8 models below, spanning Commercial Chef 0.6 Cu. Ft. Countertop Microwave Oven up to Sharp R-21LCFS Medium-Duty Commercial Microwave Oven with Dial Timer, with a focus on measuring button height and width from official dimensions or direct inspection, and cross-checked label clarity and one-press behavior against the current listing data rather than repeating manufacturer marketing claims.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Microwaves With Large Button</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Microwaves With Large Buttons</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Microwaves With Large Buttons</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=microwaves+with+large+buttons&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
