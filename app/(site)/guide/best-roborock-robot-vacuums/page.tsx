...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-roborock-robot-vacuums";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-roborock-robot-vacuums",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-roborock-robot-vacuums");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-roborock-robot-vacuums` },
    about: [{"@type":"Thing","name":"Best Roborock Robot Vacuums"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Roborock Robot Vacuums", item: `${SITE_URL}/guide/best-roborock-robot-vacuums` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-roborock-robot-vacuums#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Roborock Robot Vacuums</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Roborock Robot Vacuums" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>The Roborock lineup now spans three distinct tiers: the budget-focused Q-series, the mid-to-upper Qrevo series, and the flagship Saros series. That range makes it easy to either overspend on features you will not use or underspend and end up frustrated by a dock that cannot keep up with a busy household. Pa suction figures get most of the marketing attention, but they do not tell the whole story on their own.</p>
          <p>We researched and compared seven current, genuinely distinct Roborock models rather than color or regional variants of the same base unit, focusing on the details that actually change daily use: suction tier relative to price, navigation type, obstacle avoidance versus mapping accuracy, mop lifting behavior, and what each dock actually requires in ongoing upkeep.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Roborock Robot Vacuum</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Roborock Robot Vacuums</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Roborock Robot Vacuums</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=roborock+robot+vacuums&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
