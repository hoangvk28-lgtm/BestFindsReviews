...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-privacy-focused-robot-vacuums";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-privacy-focused-robot-vacuums",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-privacy-focused-robot-vacuums");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-privacy-focused-robot-vacuums` },
    about: [{"@type":"Thing","name":"Best Privacy-Focused Robot Vacuums"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Privacy-Focused Robot Vacuums", item: `${SITE_URL}/guide/best-privacy-focused-robot-vacuums` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-privacy-focused-robot-vacuums#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Privacy-Focused Robot Vacuums</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Privacy-Focused Robot Vacuums" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A growing number of robot vacuums navigate using an onboard camera, which means a live video feed exists somewhere between the robot and the app, even if it is only used for mapping. Every model in this guide takes a different approach: they navigate with LiDAR, a spinning laser sensor that builds a map from distance measurements instead of images. There is no camera to point at your living room and no video stream to worry about being intercepted or misused.</p>
          <p>That said, LiDAR-only navigation is not the same thing as zero data collection. These robots still connect to an app, often require an account, and can store floor plan maps and cleaning history in the cloud depending on the brand. We researched what each model design implies about privacy, but buyers should still check the manufacturer current privacy policy for specifics on account requirements, map storage, and data deletion before assuming a LiDAR robot is automatically a privacy-neutral purchase.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Privacy-Focused Robot Vacuum</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Privacy-Focused Robot Vacuums</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Privacy-Focused Robot Vacuums</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=privacy-focused+robot+vacuums&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
