...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-robot-pool-vacuums";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-robot-pool-vacuums",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-robot-pool-vacuums");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-robot-pool-vacuums` },
    about: [{"@type":"Thing","name":"Best Robot Pool Vacuums"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Robot Pool Vacuums", item: `${SITE_URL}/guide/best-robot-pool-vacuums` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-robot-pool-vacuums#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Robot Pool Vacuums</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Robot Pool Vacuums" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Robot pool vacuums are a different category from indoor floor robots. Instead of navigating carpet and hardwood, they have to climb pool walls, track the waterline for scum buildup, and survive being submerged in chlorinated water for hours at a time. Above-ground and in-ground pools also put very different demands on a unit, from flat-bottom coverage to full wall-climbing ability.</p>
          <p>We looked at eight current, distinct models rather than color variants of the same cleaner, and focused on the details that actually change day-to-day use: which pool types each one is rated for, how much of the wall and waterline it actually reaches, how long it runs on a charge, how heavy it is to lift out and empty, and how easy the filter basket is to access and clean.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Robot Pool Vacuum</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Robot Pool Vacuums</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Robot Pool Vacuums</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=robot+pool+vacuums&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
