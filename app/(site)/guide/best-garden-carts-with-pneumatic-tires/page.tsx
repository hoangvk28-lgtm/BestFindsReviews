...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-garden-carts-with-pneumatic-tires";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-garden-carts-with-pneumatic-tires",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-garden-carts-with-pneumatic-tires");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-garden-carts-with-pneumatic-tires` },
    about: [{"@type":"Thing","name":"Best Garden Carts With Pneumatic Tires (2026)"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Garden Carts With Pneumatic Tires (2026)", item: `${SITE_URL}/guide/best-garden-carts-with-pneumatic-tires` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-garden-carts-with-pneumatic-tires#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Garden Carts With Pneumatic Tires (2026)</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Garden Carts With Pneumatic Tires (2026)" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Pneumatic, air-filled tires cushion a garden cart's ride over roots, gravel, and uneven ground far better than a solid or no-flat tire, at the cost of needing occasional pressure checks and carrying some puncture risk. Manufacturers that use no-flat or solid tires almost always call that out explicitly as a selling point, since it's a genuine differentiator, so a listing's silence on tire type is itself a useful signal.</p>
          <p>We checked all eight of these current carts for explicit tire-type language rather than assuming. Only one listing here, the Sekey model, names rubber tires directly. The rest are steel utility wagons and dump carts in a weight class where pneumatic tires are the standard fitment and no-flat construction would normally be marketed by name, but we're flagging that as an inference, not a confirmed spec, so double-check the current listing's tire section before ordering.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Garden Carts With Pneumatic Tires (2026)</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Garden Carts With Pneumatic Tires (2026)</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Garden Carts With Pneumatic Tires (2026)</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=garden+cart+pneumatic+tires&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
