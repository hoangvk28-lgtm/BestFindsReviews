...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-lawn-mowers-under-200";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-lawn-mowers-under-200",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-lawn-mowers-under-200");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-lawn-mowers-under-200` },
    about: [{"@type":"Thing","name":"Best Lawn Mowers Under $200"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Lawn Mowers Under $200", item: `${SITE_URL}/guide/best-lawn-mowers-under-200` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-lawn-mowers-under-200#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Lawn Mowers Under $200</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Lawn Mowers Under $200" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Under $200 covers three very different power sources, small gas mowers, corded or battery electrics, and manual reel mowers, and each one asks for a different tradeoff between price, effort, and yard size. We compared complete ready-to-mow price, deck width, cut quality, and safety and warranty basics rather than ranking by price alone.</p>
          <p>This site also publishes a separate Best Lawn Mowers Under $100 guide for the tightest budgets and a Best Lawn Mowers Under $300 guide for buyers who can stretch past this tier for self-propel, larger decks, or more battery capacity. Use this guide if a complete, ready-to-mow setup under $200 is the goal.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Lawn Mowers Under $200</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Lawn Mowers Under $200</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Lawn Mowers Under $200</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=lawn+mowers+under+200&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
