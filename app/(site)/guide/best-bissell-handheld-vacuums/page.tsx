...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-bissell-handheld-vacuums";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-bissell-handheld-vacuums",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-bissell-handheld-vacuums");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-bissell-handheld-vacuums` },
    about: [{"@type":"Thing","name":"Best Bissell Handheld Vacuums (2026)"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Bissell Handheld Vacuums (2026)", item: `${SITE_URL}/guide/best-bissell-handheld-vacuums` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-bissell-handheld-vacuums#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Bissell Handheld Vacuums (2026)</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Bissell Handheld Vacuums (2026)" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Our initial research list for Bissell handheld vacuums turned out to include several mismatched products, some listings were actually Shark-branded rather than genuine Bissell, and two additional ASINs failed to return current listing data entirely. After verifying against actual current data, we found only one genuine Bissell handheld vacuum currently confirmed available.</p>
          <p>Rather than pad this list with products that aren't actually Bissell, we're being direct about what we verified. We've included the one confirmed genuine Bissell pick, plus clearly labeled comparison points from other brands for context.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Bissell Handheld Vacuums (2026)</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Bissell Handheld Vacuums (2026)</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Bissell Handheld Vacuums (2026)</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=bissell+handheld+vacuums&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
