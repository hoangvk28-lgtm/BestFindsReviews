...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/garden-cart-vs-wheelbarrow";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/garden-cart-vs-wheelbarrow",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("garden-cart-vs-wheelbarrow");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/garden-cart-vs-wheelbarrow` },
    about: [{"@type":"Thing","name":"Garden Cart vs. Wheelbarrow: Which Should You Buy? (2026)"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Garden Cart vs. Wheelbarrow: Which Should You Buy? (2026)", item: `${SITE_URL}/guide/garden-cart-vs-wheelbarrow` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/garden-cart-vs-wheelbarrow#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Garden Cart vs. Wheelbarrow: Which Should You Buy? (2026)</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Garden Cart vs. Wheelbarrow: Which Should You Buy? (2026)" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A 4-wheel garden cart and a 2-wheel wheelbarrow-style cart solve overlapping but genuinely different problems. A 4-wheel cart distributes weight across four contact points, so it stays level and stable when loaded heavy and parked, while a 2-wheel design pivots on a single axle, trading some stability for a tighter turning radius and easier maneuvering around raised beds, fence corners, or narrow side-yard gates.</p>
          <p>We lined up four genuine 4-wheel garden carts against four 2-wheel wheelbarrow-style carts, all currently available, to make the tradeoff concrete rather than theoretical. The verdict isn't universal: a 4-wheel cart wins for hauling heavy, bulky loads across open, relatively flat ground, while a 2-wheel wheelbarrow-style design wins when you need to squeeze through tight spaces or tip a load precisely at the edge of a bed.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Garden Cart vs. Wheelbarrow: Which Should You Buy? (2026)</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Garden Cart vs. Wheelbarrow: Which Should You Buy? (2026)</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Garden Cart vs. Wheelbarrow: Which Should You Buy? (2026)</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=garden+cart+vs+wheelbarrow&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
