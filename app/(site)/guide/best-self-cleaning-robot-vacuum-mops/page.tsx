...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-self-cleaning-robot-vacuum-mops";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-self-cleaning-robot-vacuum-mops",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-self-cleaning-robot-vacuum-mops");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-self-cleaning-robot-vacuum-mops` },
    about: [{"@type":"Thing","name":"Best Self-Cleaning Robot Vacuum Mops"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Self-Cleaning Robot Vacuum Mops", item: `${SITE_URL}/guide/best-self-cleaning-robot-vacuum-mops` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-self-cleaning-robot-vacuum-mops#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Self-Cleaning Robot Vacuum Mops</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Self-Cleaning Robot Vacuum Mops" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Self-cleaning robot vacuum mops all promise to wash their own mop pads and empty their own dustbins, but the mechanism behind that promise varies a lot from model to model. A roller mop, a spinning pad, and a vibrating pad all pick up and release dirt differently, and the dock wash water temperature has a real effect on how clean the pad actually gets between runs.</p>
          <p>We researched eight current models across a wide price range, from budget vacuum-and-mop combos under $150 to premium docks with hot-water washing and heated drying. We focused on the details that determine whether a self-cleaning dock is genuinely low-maintenance or just shifts the cleaning chore from the floor to a tray you still have to scrub by hand every week.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Self-Cleaning Robot Vacuum Mop</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Self-Cleaning Robot Vacuum Mops</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Self-Cleaning Robot Vacuum Mops</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=self-cleaning+robot+vacuum+mops&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
