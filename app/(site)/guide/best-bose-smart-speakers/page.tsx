...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-bose-smart-speakers";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-bose-smart-speakers",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-bose-smart-speakers");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-bose-smart-speakers` },
    about: [{"@type":"Thing","name":"Best Bose Smart Speakers (2026)"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Bose Smart Speakers (2026)", item: `${SITE_URL}/guide/best-bose-smart-speakers` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-bose-smart-speakers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Bose Smart Speakers (2026)</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Bose Smart Speakers (2026)" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Note: Bose's current smart speaker lineup centers on the new 2026 Lifestyle Ultra Speaker and the Home Speaker 500, which currently shows as unavailable on Amazon, verify directly with Bose if you want that specific model. The Bose Portable Smart Speaker, a genuine dual Alexa and Google Assistant portable option, was not accessible through our current product data source, verify its current listing directly if portability matters to you. Bose soundbars (Soundbar 300, Smart Dolby Atmos Soundbar) are TV-focused home theater products and are not covered in this speaker-focused comparison.</p>
          <p>We compared these four current picks by audio technology and Amazon availability. Prices here span a wide range across this lineup.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Bose Smart Speakers (2026)</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Bose Smart Speakers (2026)</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Bose Smart Speakers (2026)</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=best+bose+smart+speakers&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
