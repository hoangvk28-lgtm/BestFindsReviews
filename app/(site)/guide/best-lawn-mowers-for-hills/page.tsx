...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-lawn-mowers-for-hills";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-lawn-mowers-for-hills",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-lawn-mowers-for-hills");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-lawn-mowers-for-hills` },
    about: [{"@type":"Thing","name":"Best Lawn Mowers for Hills"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Lawn Mowers for Hills", item: `${SITE_URL}/guide/best-lawn-mowers-for-hills` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-lawn-mowers-for-hills#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Lawn Mowers for Hills</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Lawn Mowers for Hills" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Sloped lawns put more demand on a mower than a flat yard does. Traction, drive type, center of gravity, and how a machine handles turns on an incline all matter more than headline power figures. We compared rear-wheel drive gas mowers, self-propelled cordless mowers, and a slope-rated robot mower on the factors that actually change how a mower behaves on a hill.</p>
          <p>Safety first: always follow the manufacturer specific slope guidance for the model you buy, including any published incline limit, and never assume a mower is rated for a slope steeper than what the manual states. This guide compares drive systems, format, and included traction features so you can match a mower to your property rather than guessing.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Lawn Mowers for Hill</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Lawn Mowers for Hills</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Lawn Mowers for Hills</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=lawn+mowers+for+hills&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
