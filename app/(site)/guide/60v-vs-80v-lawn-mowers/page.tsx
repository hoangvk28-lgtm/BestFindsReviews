...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/60v-vs-80v-lawn-mowers";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/60v-vs-80v-lawn-mowers",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("60v-vs-80v-lawn-mowers");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/60v-vs-80v-lawn-mowers` },
    about: [{"@type":"Thing","name":"60V vs 80V Lawn Mowers"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "60V vs 80V Lawn Mowers", item: `${SITE_URL}/guide/60v-vs-80v-lawn-mowers` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/60v-vs-80v-lawn-mowers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">60V vs 80V Lawn Mowers</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="60V vs 80V Lawn Mowers" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A higher voltage number is not automatically a stronger or longer-running mower. Voltage is electrical pressure, not stored energy, and a 60V pack with a larger amp-hour rating can hold more watt-hours than an 80V pack with a smaller one. Nominal volts multiplied by amp-hours gives the approximate watt-hours, and that number, not the voltage label, is what predicts runtime and cutting capacity under real grass load.</p>
          <p>We compared included battery capacity, deck width, self-propel drive load, operating weight, recharge workflow, and price across current 60V and 80V mowers so you can match the platform to your lawn instead of assuming the bigger number wins by default.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a 60V vs 80V Lawn Mower</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These 60V vs 80V Lawn Mowers</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right 60V vs 80V Lawn Mowers</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=60v+vs+80v+lawn+mowers&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
