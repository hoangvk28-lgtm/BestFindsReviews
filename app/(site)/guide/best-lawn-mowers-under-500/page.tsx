...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-lawn-mowers-under-500";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-lawn-mowers-under-500",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-lawn-mowers-under-500");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-lawn-mowers-under-500` },
    about: [{"@type":"Thing","name":"Best Lawn Mowers Under $500"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Lawn Mowers Under $500", item: `${SITE_URL}/guide/best-lawn-mowers-under-500` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-lawn-mowers-under-500#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Lawn Mowers Under $500</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Lawn Mowers Under $500" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A $500 ceiling still spans push electric mowers under $250, self-propelled electric mowers near $450, and gas self-propelled mowers in between, so the format you get for the money varies far more than the price alone suggests. Instead of ranking by price, we grouped these six current models by what a buyer actually gives up or gains as the price climbs from around $250 toward the $500 line.</p>
          <p>We compared deck width, propulsion, power source, and the complete price of what ships in the box (battery and charger included, or gas engine ready to start) against buyer ratings and review volume, so you can see where the extra spending actually buys capability rather than just a bigger number on the listing.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Lawn Mowers Under $500</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Lawn Mowers Under $500</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Lawn Mowers Under $500</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=lawn+mowers+under+500&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
