...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-heavy-duty-rolling-garden-seats";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-heavy-duty-rolling-garden-seats",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-heavy-duty-rolling-garden-seats");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-heavy-duty-rolling-garden-seats` },
    about: [{"@type":"Thing","name":"Best Heavy-Duty Rolling Garden Seats (2026)"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Heavy-Duty Rolling Garden Seats (2026)", item: `${SITE_URL}/guide/best-heavy-duty-rolling-garden-seats` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-heavy-duty-rolling-garden-seats#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Heavy-Duty Rolling Garden Seats (2026)</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Heavy-Duty Rolling Garden Seats (2026)" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Seat height and transfer, Wheel/terrain behavior, and Stability and rated load rarely show up as clear differentiators in a standard listing for heavy-duty rolling garden seats, even though they're often the real reason one pick performs better than another once it's in actual use. Weight Capacity alone in this lineup spans from 400 lbs on the VEVOR Garden Seat with Wheels up to 500 lbs on the VEVOR 500 LBS Steel Garden Cart with Work Seat, a real difference worth checking against your own use case.</p>
          <p>The 7 picks compared here were evaluated with checking not rank by advertised weight capacity alone as the starting point, not an afterthought. Prices here span from the most affordable pick, the FLINTER Garden Cart with Seat, to the priciest pick, the BESTHLS Garden Cart with Seat on Wheels Rolling Gardening Stool with. We also cross-checked Materials and weather resistance and assembly, storage and replacement parts against the actual listing data for each one.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Heavy-Duty Rolling Garden Seats (2026)</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Heavy-Duty Rolling Garden Seats (2026)</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Heavy-Duty Rolling Garden Seats (2026)</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=Heavy-Duty+Rolling+Garden+Seats&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
