...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-coffee-makers-with-thermal-carafes";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-coffee-makers-with-thermal-carafes",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-coffee-makers-with-thermal-carafes");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-coffee-makers-with-thermal-carafes` },
    about: [{"@type":"Thing","name":"Best Coffee Makers With Thermal Carafes"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Coffee Makers With Thermal Carafes", item: `${SITE_URL}/guide/best-coffee-makers-with-thermal-carafes` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-coffee-makers-with-thermal-carafes#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Coffee Makers With Thermal Carafes</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Coffee Makers With Thermal Carafes" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Thermal carafe guides usually stop at heat retention and move on, treating every insulated carafe as functionally the same. In practice the lid design, how much coffee gets trapped inside when you pour, and what a replacement carafe costs if it cracks vary a lot between models.</p>
          <p>We compared four current thermal carafe coffee makers and looked past the headline heat-retention claim to the details that show up in daily use: one-handed pouring control, how much coffee a lid design traps versus a simple glass lid, how involved disassembling the lid is for cleaning, and replacement carafe price and availability.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Coffee Makers With Thermal Carafe</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Coffee Makers With Thermal Carafes</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Coffee Makers With Thermal Carafes</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=coffee+makers+with+thermal+carafe&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
