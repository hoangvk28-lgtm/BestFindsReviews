...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-cold-brew-coffee-makers";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-cold-brew-coffee-makers",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-cold-brew-coffee-makers");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-cold-brew-coffee-makers` },
    about: [{"@type":"Thing","name":"Best Cold Brew Coffee Makers"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Cold Brew Coffee Makers", item: `${SITE_URL}/guide/best-cold-brew-coffee-makers` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-cold-brew-coffee-makers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Cold Brew Coffee Makers</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Cold Brew Coffee Makers" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Cold brew pitcher capacity is usually advertised as if every ounce becomes a drinkable serving, but cold brew is a concentrate meant to be diluted with water, milk, or ice before drinking, and a portion of what you brew never leaves the grounds. Most comparisons repeat the fluid capacity of the pitcher as the number of servings, which overstates what you actually get to drink.</p>
          <p>We compared four current cold brew makers on realistic yield after dilution and sediment loss, how well each one fits and seals in a fridge door or shelf, how difficult grounds removal and cleaning really are, and the split between slow manual steep-and-strain systems and fast electric extraction systems, since the tradeoffs between those two approaches are genuinely different.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Cold Brew Coffee Maker</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Cold Brew Coffee Makers</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Cold Brew Coffee Makers</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=cold+brew+coffee+makers&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
