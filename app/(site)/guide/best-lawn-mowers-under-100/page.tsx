...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-lawn-mowers-under-100";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-lawn-mowers-under-100",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-lawn-mowers-under-100");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-lawn-mowers-under-100` },
    about: [{"@type":"Thing","name":"Best Lawn Mowers Under $100"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Lawn Mowers Under $100", item: `${SITE_URL}/guide/best-lawn-mowers-under-100` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-lawn-mowers-under-100#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Lawn Mowers Under $100</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Lawn Mowers Under $100" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Search for a lawn mower under $100 and the honest answer is that the powered options at this price are limited. Most full-size gas and battery mowers cross $100 quickly once you add a real deck width, a usable battery pack, or a self-propel system, so the mowers that stay under this line are mostly manual reel mowers and a small number of compact corded electrics.</p>
          <p>We looked at what is actually available under $100 right now rather than padding the list with items priced just over the line. That means fewer entries than a typical roundup, a reel mower at the top for anyone with a small, flat, obstacle-light lawn, and clear notes on where a slightly higher budget buys real capability.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Lawn Mowers Under $100</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Lawn Mowers Under $100</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Lawn Mowers Under $100</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=lawn+mowers+under+100&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
