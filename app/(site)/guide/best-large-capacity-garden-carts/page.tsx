...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-large-capacity-garden-carts";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-large-capacity-garden-carts",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-large-capacity-garden-carts");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-large-capacity-garden-carts` },
    about: [{"@type":"Thing","name":"Best Large-Capacity Garden Carts (2026)"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Large-Capacity Garden Carts (2026)", item: `${SITE_URL}/guide/best-large-capacity-garden-carts` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-large-capacity-garden-carts#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Large-Capacity Garden Carts (2026)</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Large-Capacity Garden Carts (2026)" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>When the job is hauling gravel, firewood, or a full load of wet soil, rated weight capacity and bed volume matter more than any other spec on a garden cart listing. A cart that tops out at 400 or 600 lbs will bottom out or strain its frame well before a 1200+ lb-rated cart even gets close to its limit.</p>
          <p>We ranked five current manually-pulled garden carts here directly by their stated capacity and bed volume, from 1400 lbs down to 800 lbs. Three other listings in our source data are electric, motor-assisted carts with lower actual pull capacity, we've flagged those separately since they don't compete on the same terms as a manual high-capacity cart.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Large-Capacity Garden Carts (2026)</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Large-Capacity Garden Carts (2026)</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Large-Capacity Garden Carts (2026)</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=large+capacity+garden+cart&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
