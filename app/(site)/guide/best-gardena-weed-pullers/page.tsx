...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-gardena-weed-pullers";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-gardena-weed-pullers",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-gardena-weed-pullers");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-gardena-weed-pullers` },
    about: [{"@type":"Thing","name":"Best Gardena Weed Pullers (2026)"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Gardena Weed Pullers (2026)", item: `${SITE_URL}/guide/best-gardena-weed-pullers` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-gardena-weed-pullers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Gardena Weed Pullers (2026)</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Gardena Weed Pullers (2026)" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Gardena's current US lineup centers on one primary Weed Puller design, so this guide leans into a closer look at that model against direct competitors rather than a long within-brand list. Shoppers comparing gardena weed pullers often assume any well-reviewed listing is a safe bet, but Current SKU count and Mechanism/material vary enough between these picks to change which one actually fits a given yard, and warranty is rarely spelled out clearly on the product page. Claw Count alone in this lineup spans from 3 claws on the Walensee Weed Puller up to 4 claws on the Grampa's Weeder Original Stand Up Weed Puller Tool with Bamboo Long, a real difference worth checking against your own use case.</p>
          <p>This roundup covers 8 current models and prioritizes building a current SKU map and decide whether a plural roundup is justified over spec-sheet comparisons alone. Prices here span from the most affordable pick, the RexWeed Stand Up Weed Puller Tool, to the priciest pick, the Grootpow WP5 Heavy Duty Weed Puller Tool. Replacement/support and Weed/soil fit were checked against the live listing data for each pick, not assumed from the box copy.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Gardena Weed Pullers (2026)</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Gardena Weed Pullers (2026)</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Gardena Weed Pullers (2026)</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=Gardena+Weed+Pullers&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
