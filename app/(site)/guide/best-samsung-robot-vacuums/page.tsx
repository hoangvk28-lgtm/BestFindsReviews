...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-samsung-robot-vacuums";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-samsung-robot-vacuums",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-samsung-robot-vacuums");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-samsung-robot-vacuums` },
    about: [{"@type":"Thing","name":"Best Samsung Robot Vacuums"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Samsung Robot Vacuums", item: `${SITE_URL}/guide/best-samsung-robot-vacuums` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-samsung-robot-vacuums#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Samsung Robot Vacuums</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Samsung Robot Vacuums" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>The current Samsung US robot vacuum lineup is smaller than what you will find from Roborock, Shark, or eufy. Instead of a dozen overlapping trims, Samsung currently sells a compact self-emptying flagship and a simpler cordless-style model, both built around its Jet Bot platform. That narrower lineup is itself useful context: if you want a wide spread of price points and feature tiers to choose between, Samsung is not the brand for that, but if you already own Samsung appliances and want them on one SmartThings app, it may still be worth a look.</p>
          <p>We evaluated the two genuinely distinct Samsung models currently available and left out several other-brand robot vacuums that showed up in the same search results but are not actually made by Samsung. Category, self-empty capability, camera and app dependence, and price at this tier were our focus, not a large lineup that does not exist.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Samsung Robot Vacuum</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Samsung Robot Vacuums</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Samsung Robot Vacuums</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=samsung+robot+vacuums&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
