...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-8-cup-coffee-makers";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-8-cup-coffee-makers",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-8-cup-coffee-makers");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-8-cup-coffee-makers` },
    about: [{"@type":"Thing","name":"Best 8-Cup Coffee Makers"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best 8-Cup Coffee Makers", item: `${SITE_URL}/guide/best-8-cup-coffee-makers` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-8-cup-coffee-makers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best 8-Cup Coffee Makers</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best 8-Cup Coffee Makers" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>An 8-cup coffee maker is sized for one or two people, since a manufacturer cup is approximately 5 fluid ounces, so 8 cups works out to roughly 40 fluid ounces total, about 5 standard 8oz mugs. It sits between a single-serve machine and the far more common 10, 12, and 14-cup carafe models.

When we pulled current listings for this size, most machines marketed loosely around the 8-cup label turned out to be 10 or 12-cup carafes with adjusted copy. We verified capacity against the stated ounces on each listing and found only two genuinely 8-cup-specific models currently available, both reviewed in full below, along with guidance on when a 10-cup machine with a good small-batch mode is the more practical buy.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a 8-Cup Coffee Maker</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These 8-Cup Coffee Makers</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right 8-Cup Coffee Makers</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=8+cup+coffee+makers&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
