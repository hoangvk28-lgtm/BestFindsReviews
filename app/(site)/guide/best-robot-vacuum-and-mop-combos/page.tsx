...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-robot-vacuum-and-mop-combos";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-robot-vacuum-and-mop-combos",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-robot-vacuum-and-mop-combos");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-robot-vacuum-and-mop-combos` },
    about: [{"@type":"Thing","name":"Best Robot Vacuum and Mop Combos"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Robot Vacuum and Mop Combos", item: `${SITE_URL}/guide/best-robot-vacuum-and-mop-combos` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-robot-vacuum-and-mop-combos#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Robot Vacuum and Mop Combos</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Robot Vacuum and Mop Combos" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Robot vacuum and mop combos have become the default recommendation for buyers who used to purchase a robot vacuum and a separate mopping robot, mostly because a single dock that empties dust, washes its own mop pads, and refills clean water removes most of the daily upkeep that made early robot mops a hassle. The catch is that most listings market suction power and mopping as one combined feature, when in practice they are two separate systems with separate strengths.</p>
          <p>We looked at eight current, distinct models rather than color variants of the same machine, spanning budget picks under $150 up to premium docks near $500, and focused on the details that actually change daily use: how well the vacuum performs separately from the mop, whether the mop pad lifts automatically on carpet, how much manual dock maintenance each system still requires, and how suction power tracks with price.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Robot Vacuum and Mop Combo</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Robot Vacuum and Mop Combos</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Robot Vacuum and Mop Combos</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=robot+vacuum+and+mop+combos&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
