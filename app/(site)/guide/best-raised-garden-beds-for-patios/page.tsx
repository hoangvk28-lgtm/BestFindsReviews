...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-raised-garden-beds-for-patios";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-raised-garden-beds-for-patios",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-raised-garden-beds-for-patios");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-raised-garden-beds-for-patios` },
    about: [{"@type":"Thing","name":"Best Raised Garden Beds for Patios"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Raised Garden Beds for Patios", item: `${SITE_URL}/guide/best-raised-garden-beds-for-patios` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-raised-garden-beds-for-patios#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Raised Garden Beds for Patios</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Raised Garden Beds for Patios" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Best Raised Garden Beds for Patios can look similar in a listing photo but differ in ways that matter once soil, water, and a full season of weather are involved: Loaded weight, Drainage routing, and Surface staining are the details most listings gloss over in favor of a generic product shot. Height alone in this lineup spans from 30 inches on the Best Choice Products 48 x 24 x 30 Inches Raised Garden Bed up to 33.7 inches on the WTFGO Elevate 33.7" Tall Raised Garden Bed with Wheels, a real difference in how much bending a gardener has to do.</p>
          <p>We compared all 8 models below, spanning OYEAL Raised Garden Bed 32" Tall Elevated Planter Boxes with Trellis at $35.14 up to Keter Urban Bloomer 12.7 Gallon Raised Garden Bed and Outdoor Herb at $103.32, with a focus on measuring complete wet weight and compare with the patio or deck's suitability, and cross-checked Sun exposure and Mobility against the current listing data rather than repeating manufacturer marketing claims.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Raised Garden Beds for Patio</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Raised Garden Beds for Patios</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Raised Garden Beds for Patios</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=Raised+Garden+Beds+for+Patios&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
