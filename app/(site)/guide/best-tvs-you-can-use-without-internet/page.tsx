...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-tvs-you-can-use-without-internet";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-tvs-you-can-use-without-internet",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-tvs-you-can-use-without-internet");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-tvs-you-can-use-without-internet` },
    about: [{"@type":"Thing","name":"Best TVs You Can Use Without Internet (2026)"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best TVs You Can Use Without Internet (2026)", item: `${SITE_URL}/guide/best-tvs-you-can-use-without-internet` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-tvs-you-can-use-without-internet#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best TVs You Can Use Without Internet (2026)</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best TVs You Can Use Without Internet (2026)" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Genuinely non-smart consumer TVs are increasingly rare, so this guide focuses on the more durable buyer problem: avoiding internet accounts, ads, and tracking while keeping a normal tuner, remote, and speakers - including smart TVs that work well used offline. Best TVs You Can Use Without Internet can look similar in a listing photo but differ in ways that matter once real daily use is involved: True non-smart status, Consumer versus commercial, and Offline operation are the details most listings gloss over in favor of a generic spec sheet. Screen Size alone in this lineup spans from 27" on the LG - 27" Class LED Full HD Smart TV Monitor with webOS up to 50" on the Westinghouse Roku TV - 50 Inch Smart TV, a real difference worth checking against your own use case.</p>
          <p>We compared all 8 models below, spanning INSIGNIA 24” Class F20 Series LED HD Smart Fire TV at $69.99 up to Samsung 32-Inch Class QLED Q8F 4K UHD Smart TV (2025 Model) Q4 AI at $397.99, with a focus on verifying that each pick is a real consumer TV rather than commercial/hospitality hardware, and cross-checked Tuner/speakers and Warranty/support against the current listing data rather than repeating manufacturer marketing claims.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a TVs You Can Use Without Internet (2026)</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These TVs You Can Use Without Internet (2026)</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right TVs You Can Use Without Internet (2026)</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=TVs+You+Can+Use+Without+Internet&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
