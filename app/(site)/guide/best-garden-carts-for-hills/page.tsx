...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-garden-carts-for-hills";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-garden-carts-for-hills",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-garden-carts-for-hills");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-garden-carts-for-hills` },
    about: [{"@type":"Thing","name":"Best Garden Carts for Hills (2026)"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Garden Carts for Hills (2026)", item: `${SITE_URL}/guide/best-garden-carts-for-hills` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-garden-carts-for-hills#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Garden Carts for Hills (2026)</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Garden Carts for Hills (2026)" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Be honest about what a manual garden cart can and can't do on a slope: none of the manual carts in this comparison have an actual braking mechanism. Control on a hill comes down to a 4-wheel frame's wider, more stable footprint, tire grip, and your own grip on the handle, not any built-in brake. A 2-wheel wheelbarrow-style cart is inherently less stable going downhill loaded, since it pivots on a single axle instead of resting on four points.</p>
          <p>This lineup includes five manual carts and three electric utility carts, and the two categories behave very differently on hills. Electric carts can add motor-assisted control and, in one case here, a named brake, genuinely relevant advantages on a slope, but they're a different category of product at a very different price point, not a drop-in swap for a manual cart. We've flagged each one clearly rather than treating them as equivalent.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Garden Carts for Hills (2026)</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Garden Carts for Hills (2026)</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Garden Carts for Hills (2026)</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=garden+cart+for+hills&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
