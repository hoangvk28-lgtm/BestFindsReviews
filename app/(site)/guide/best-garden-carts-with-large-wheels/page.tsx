...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-garden-carts-with-large-wheels";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-garden-carts-with-large-wheels",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-garden-carts-with-large-wheels");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-garden-carts-with-large-wheels` },
    about: [{"@type":"Thing","name":"Best Garden Carts With Large Wheels (2026)"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Garden Carts With Large Wheels (2026)", item: `${SITE_URL}/guide/best-garden-carts-with-large-wheels` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-garden-carts-with-large-wheels#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Garden Carts With Large Wheels (2026)</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Garden Carts With Large Wheels (2026)" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Wheel size is one of the least-advertised specs on a garden cart listing, yet it's often the difference between a cart that rolls smoothly over roots, ruts, and gravel and one that gets stuck in every soft patch. Bigger wheels, especially pneumatic ones, generally clear obstacles with less pushing effort than small hard casters, but plenty of listings bury or omit the exact diameter entirely.</p>
          <p>We looked at six current manually-pulled garden carts here, all rated for real hauling loads with wheel setups sized to match. Two listings that turned up in our source data are actually electric, motor-driven carts rather than manual pull carts with oversized wheels, we've flagged both clearly below rather than pretend they fit this comparison.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Garden Carts With Large Wheels (2026)</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Garden Carts With Large Wheels (2026)</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Garden Carts With Large Wheels (2026)</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=garden+cart+large+wheels&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
