...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-rear-wheel-drive-lawn-mowers";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-rear-wheel-drive-lawn-mowers",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-rear-wheel-drive-lawn-mowers");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-rear-wheel-drive-lawn-mowers` },
    about: [{"@type":"Thing","name":"Best Rear-Wheel-Drive Lawn Mowers"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Rear-Wheel-Drive Lawn Mowers", item: `${SITE_URL}/guide/best-rear-wheel-drive-lawn-mowers` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-rear-wheel-drive-lawn-mowers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Rear-Wheel-Drive Lawn Mowers</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Rear-Wheel-Drive Lawn Mowers" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Rear-wheel drive is a specific mechanical choice, not a marketing synonym for self-propelled. Putting the drive wheels behind the deck shifts weight toward the rear axle, which changes how a mower climbs a slope, holds a line on uneven ground, and reacts when the drive is released partway across a hill.</p>
          <p>We limited this list to mowers whose own product titles confirm rear-wheel drive, since front-wheel and all-wheel drive models are frequently mixed into the same shopping results. We compare hill traction, turning effort, reverse drag, low-speed control, and how the rear weight bias affects lifting and transport.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Rear-Wheel-Drive Lawn Mower</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Rear-Wheel-Drive Lawn Mowers</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Rear-Wheel-Drive Lawn Mowers</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=rear+wheel+drive+lawn+mowers&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
