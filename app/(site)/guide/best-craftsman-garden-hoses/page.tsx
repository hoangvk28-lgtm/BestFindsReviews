...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-craftsman-garden-hoses";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-craftsman-garden-hoses",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-craftsman-garden-hoses");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-craftsman-garden-hoses` },
    about: [{"@type":"Thing","name":"Best Craftsman Garden Hoses (2026)"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Craftsman Garden Hoses (2026)", item: `${SITE_URL}/guide/best-craftsman-garden-hoses` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-craftsman-garden-hoses#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Craftsman Garden Hoses (2026)</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Craftsman Garden Hoses (2026)" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Best Craftsman Garden Hoses can look similar in a listing photo but differ in ways that matter once real daily use is involved: Current US product pool, Inside diameter and fitting restriction, and Dry versus filled handling are the details most listings gloss over in favor of a generic spec sheet. Length alone in this lineup spans from 50 ft on the Bionic Steel PRO 50 Ft Metal Garden Hose with Brass Nozzle up to 100 ft on the Tuzoc Rcfunkid Garden Hose 100FT No-Kink Metal Hose With 10 Function Nozzle, a real difference worth checking against your own use case.</p>
          <p>We compared all 8 models below, spanning Relxitl Garden Hose at $28.43 up to Rosy Earth 100 FT Garden Hose Expandable - 304 Stainless Steel Water at $55.97, with a focus on building a current model map by material, length, diameter and intended use, and cross-checked Working pressure and measured flow and Repairability and warranty against the current listing data rather than repeating manufacturer marketing claims.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Craftsman Garden Hoses (2026)</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Craftsman Garden Hoses (2026)</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Craftsman Garden Hoses (2026)</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=Craftsman+Garden+Hoses&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
