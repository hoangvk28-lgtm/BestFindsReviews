...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-compact-coffee-makers";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-compact-coffee-makers",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-compact-coffee-makers");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-compact-coffee-makers` },
    about: [{"@type":"Thing","name":"Best Compact Coffee Makers"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Compact Coffee Makers", item: `${SITE_URL}/guide/best-compact-coffee-makers` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-compact-coffee-makers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Compact Coffee Makers</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Compact Coffee Makers" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Most compact coffee maker roundups rank models by their published closed-box width and call it a day, then tack on a line about dorm or RV use. That number tells you almost nothing about whether the machine actually fits where you want to put it, because a coffee maker needs open space above and around it to load water, swap a mug, or lift a reservoir out for cleaning.</p>
          <p>We compared four current compact models on operational footprint rather than just box width: how much clearance the lid and reservoir need when open, whether a normal mug fits under the spout without tilting, how each performs on a single cup versus its stated maximum, and where cord storage and spill risk change the daily experience.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Compact Coffee Maker</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Compact Coffee Makers</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Compact Coffee Makers</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=mini+coffee+makers&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
