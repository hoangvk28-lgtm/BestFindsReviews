...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-stihl-electric-pruning-shears";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-stihl-electric-pruning-shears",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-stihl-electric-pruning-shears");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-stihl-electric-pruning-shears` },
    about: [{"@type":"Thing","name":"Best Stihl Electric Pruning Shears (2026)"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Stihl Electric Pruning Shears (2026)", item: `${SITE_URL}/guide/best-stihl-electric-pruning-shears` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-stihl-electric-pruning-shears#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Stihl Electric Pruning Shears (2026)</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Stihl Electric Pruning Shears (2026)" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>STIHL doesn't currently sell a dedicated electric bypass pruning shear in the US market, the kind of scissor-action tool most other brands in this category offer. What does surface in this search is the STIHL GTA 26, a battery-powered mini chainsaw-style wood cutter, a genuinely different tool that cuts with a small chain rather than a shear blade.</p>
          <p>We're covering this honestly rather than forcing a comparison that doesn't exist: if you specifically want a STIHL-branded tool for pruning-adjacent yard work, the GTA 26 is what's currently available, but it's not a pruning shear in the traditional sense.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Stihl Electric Pruning Shears (2026)</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Stihl Electric Pruning Shears (2026)</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Stihl Electric Pruning Shears (2026)</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=stihl+electric+pruning+shears&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
