...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-coffee-makers-for-hard-water";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-coffee-makers-for-hard-water",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-coffee-makers-for-hard-water");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-coffee-makers-for-hard-water` },
    about: [{"@type":"Thing","name":"Best Coffee Makers for Hard Water"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Coffee Makers for Hard Water", item: `${SITE_URL}/guide/best-coffee-makers-for-hard-water` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-coffee-makers-for-hard-water#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Coffee Makers for Hard Water</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Coffee Makers for Hard Water" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Water hardness is commonly measured in grains per gallon, and water above roughly 7 grains per gallon is generally considered hard. Hard water leaves mineral scale inside the internal tubing and heating element of a coffee maker, and most buying guides mention descale alerts without checking whether descaling is actually easy to do on a given machine.</p>
          <p>We compared five current models on filter availability, whether descaling requires removable and visible parts or awkward internal access, typical descale interval, and what each manufacturer actually recommends for descaling rather than relying on generic vinegar folklore that some manufacturers explicitly warn against.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Coffee Makers for Hard Water</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Coffee Makers for Hard Water</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Coffee Makers for Hard Water</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=coffee+makers+for+hard+water&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
