...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-self-propelled-lawn-mowers";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-self-propelled-lawn-mowers",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-self-propelled-lawn-mowers");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-self-propelled-lawn-mowers` },
    about: [{"@type":"Thing","name":"Best Self-Propelled Lawn Mowers"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Self-Propelled Lawn Mowers", item: `${SITE_URL}/guide/best-self-propelled-lawn-mowers` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-self-propelled-lawn-mowers#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Self-Propelled Lawn Mowers</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Self-Propelled Lawn Mowers" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A self-propelled mower is not just a push mower with a motor helping the wheels along. Drive systems differ by which wheels pull (front-wheel drive, rear-wheel drive, or all-wheel drive), how the speed is controlled at the handle, and how much of that drive power is left over once a slope or a full bag adds resistance. Two mowers with the same headline speed can feel completely different once the ground tilts or the grass gets thick.</p>
          <p>Most buyers move up from a push mower once pushing itself becomes the tiring part, not the cutting. That usually happens on lawns with slopes, on lawns over roughly a quarter acre, or with anyone who mows regularly and wants the drive to do the walking work. We compare drive type, control feel at low speed, turning effort, and how traction holds up as the bag fills, so you can match the mower to the property instead of the spec sheet.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Self-Propelled Lawn Mower</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Self-Propelled Lawn Mowers</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Self-Propelled Lawn Mowers</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=self-propelled+lawn+mowers&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
