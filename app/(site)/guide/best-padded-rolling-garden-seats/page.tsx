...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/best-padded-rolling-garden-seats";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/best-padded-rolling-garden-seats",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("best-padded-rolling-garden-seats");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/best-padded-rolling-garden-seats` },
    about: [{"@type":"Thing","name":"Best Padded Rolling Garden Seats (2026)"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Best Padded Rolling Garden Seats (2026)", item: `${SITE_URL}/guide/best-padded-rolling-garden-seats` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/best-padded-rolling-garden-seats#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Best Padded Rolling Garden Seats (2026)</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Best Padded Rolling Garden Seats (2026)" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>Shoppers comparing padded rolling garden seats often assume any well-reviewed listing is a safe bet, but Seat height and transfer and Wheel/terrain behavior vary enough between these picks to change which one actually fits a given yard, and stability and rated load is rarely spelled out clearly on the product page. Weight Capacity alone in this lineup spans from 330 lbs on the Masiunoz Rolling Garden Seat with Wheels up to 500 lbs on the Saker Rolling Garden Cart with Seat & Wheels 15.7" EVA Padded Stool, a real difference worth checking against your own use case.</p>
          <p>This roundup covers 8 current models and prioritizes measuring cushion thickness under load, not unloaded foam thickness over spec-sheet comparisons alone. Prices here span from the most affordable pick, the YPSLZY Garden Rolling Work Seat with Wheels, to the priciest pick, the Saker Rolling Garden Cart with Seat & Wheels 15.7" EVA Padded Stool. Materials and weather resistance and Assembly, storage and replacement parts were checked against the live listing data for each pick, not assumed from the box copy.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Padded Rolling Garden Seats (2026)</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Padded Rolling Garden Seats (2026)</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Padded Rolling Garden Seats (2026)</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=Padded+Rolling+Garden+Seats&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
