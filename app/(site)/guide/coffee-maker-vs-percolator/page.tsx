...
import {
  ...
  howToChoose,
  type GuideProduct,
} from "@/data/guides/coffee-maker-vs-percolator";
...
export const metadata: Metadata = buildMetadata({
  ...
  path: "/guide/coffee-maker-vs-percolator",
...
export default async function Page() {
  const guide = await getPublicGuideBySlug("coffee-maker-vs-percolator");
  ...
  const articleSchema = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `${SITE_URL}/guide/coffee-maker-vs-percolator` },
    about: [{"@type":"Thing","name":"Coffee Maker vs Percolator"}],
  };
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Coffee Maker vs Percolator", item: `${SITE_URL}/guide/coffee-maker-vs-percolator` },
    ],
  };
  ...
  const itemListSchema = {
    ...
    itemListElement: products.map((p) => ({
      ...
      position: p.rank,
      name: `${p.name} - ${p.badge}`,
      url: `${SITE_URL}/guide/coffee-maker-vs-percolator#${p.id}`,
    })),
  };
    ...
    <>
      ...
      <Container className="py-12 max-w-4xl">
        <nav className="flex items-center gap-1.5 text-xs text-ink-muted mb-6">
          ...
          <span>/</span>
          <span className="text-ink">Coffee Maker vs Percolator</span>
        </nav>
        ...
        <div className="rounded-2xl overflow-hidden border border-border mb-10 bg-bg">
          ...
          <div className="md:hidden h-56 sm:h-64 flex items-center justify-center">
            <Image src={heroImg} alt="Coffee Maker vs Percolator" width={900} height={420} className="w-full h-full object-contain" priority unoptimized />
          </div>
        ...
        <section className="mb-10 space-y-4 text-base text-ink-secondary leading-relaxed">
          <p>A drip coffee maker and a percolator use fundamentally different mechanisms to brew, not just different designs. A drip machine passes hot water through the grounds one time and stops. A percolator repeatedly cycles boiling water up through the grounds and back down, again and again, until the cycle is switched off or times out, which changes both flavor and over-extraction risk.</p>
          <p>We compared two current drip machines and two current electric percolators on the details that follow from that mechanism difference: brew time and taste consistency, upfront cost, exposed-metal safety considerations, and which one actually fits a larger group, camping trip, or off-grid situation better.</p>
        </section>

        {buyingCriteria.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">{buyingCriteria.length} Criteria to Look For Before Buying a Coffee Maker vs Percolator</h2>
        ...
        {howWeEvaluated.length > 0 && (
          <section className="mb-12 p-6 rounded-2xl border border-border bg-white">
            <h2 className="text-xl font-bold text-ink mb-1 tracking-tight">How We Evaluated These Coffee Maker vs Percolator</h2>
        ...
        {howToChoose.length > 0 && (
          <section className="mb-14">
            <h2 className="text-2xl font-bold text-ink mb-6 tracking-tight">How to Choose the Right Coffee Maker vs Percolator</h2>
        ...
        <section className="mb-14 p-6 rounded-2xl flex flex-col items-center text-center" style={{ background: "linear-gradient(135deg, #FF9900 0%, #e68900 100%)" }}>
          ...
          <a href={`https://www.amazon.com/s?k=coffee+maker+vs+percolator&tag=worthrated-20`} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white font-bold text-sm hover:bg-gray-50 transition-colors" style={{ color: "#FF9900" }}>
            Shop on Amazon &rarr;
          </a>
        </section>
